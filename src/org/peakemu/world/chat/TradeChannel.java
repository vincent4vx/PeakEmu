...
public class TradeChannel implements ChatChannel{
    ...
    public TradeChannel(SessionHandler sessionHandler, ChatConfig config) {
    ...
    public void onMessage(Player sender, String message) {
        ...
        if(lastMessage + config.getTradeFloodTime() > System.currentTimeMillis()){
            sender.send(new InfoMessage(InfoMessage.Type.INFO).addMessage(115, (config.getTradeFloodTime() - (System.currentTimeMillis() - lastMessage)) / 1000));
            return;
        }
